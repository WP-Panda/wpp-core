...
/**
 * Class Wpp_Custom_Taxonomy
 * Регистрация таксономий
 * @version 1.0.5
 */
if ( ! class_exists( 'Wpp_Custom_Taxonomy' ) ) :
	...
	class Wpp_Custom_Taxonomy {
		...
		public static function init() {
			...
			if ( class_exists( 'WPP_Tax_Term_Img' ) ) {
				add_filter( 'wpp_tax_imgs_targets', [ __CLASS__, 'images_for_taxonomy' ], 50 );
			}
		}
		...
		private static function post_types() {
			return apply_filters( 'wpp_register_post_types', self::$post_types );
		}
		 ...
		 * @return array
		 */
		private static function post_types_ru_labels(
			$nominative,
			$genitive,
			$dative,
			$instrumental,
		 ...
		 * @return array
		 */
		private static function taxonomy_labels( $args ) {
			extract( $args );
			...
			return [
				...
				'menu_name'         => sprintf( __( '%s', 'wpp-fr' ), $plural ),
				'back_to_items'     => sprintf( __( '%s', '← Back to %s' ), $plural ),
			];
		}
		 ...
		 * @return array
		 */
		private static function taxonomy_ru_labels( $args ) {
			extract( $args );
			...
			return [
				...
				'menu_name'                  => sprintf( __( '%s', 'wpp-fr' ), $plural ),
				'back_to_items'              => sprintf( __( '%s', '← Назад к %s' ), $plural ),
				'popular_items'              => sprintf( __( 'Популярные %s' ), $plural ),
				// для таксономий без иерархий
				'separate_items_with_commas' => sprintf( __( 'Разделяйте %s запятыми' ), $plural ),
				'add_or_remove_items'        => sprintf( __( 'Добавить или удалить %s' ), $plural ),
				'choose_from_most_used'      => sprintf( __( 'Выбрать из часто используемых %s' ), $plural ),
				'not_found'                  => sprintf( __( '%s не найдено' ), $plurals ),
			];
		}
		...
		private static function taxonomies() {
			return apply_filters( 'wpp_register_taxonomies', self::$taxonomies );
		}
		...
		public static function custom_post_type() {
			...
			if ( ! empty( $post_types ) ) :
				...
				foreach ( $post_types as $post_type => $args ) :
					...
					if ( ! empty( $args['cir'] ) && 'ru_RU' === get_locale() ) {
						/**
						 * @param $nominative     string - именительный падеж кто? что?
						 * @param $genitive       string - родительный кого? чего?
						 * @param $dative         string - дательный кого? что?
						 ...
						 * @param $plurals        string - множественное 2
						 */
						$args['labels'] = self::post_types_ru_labels( $args['single'],
							$args['genitive'],
							$args['dative'],
							$args['instrumental'],
							$args['plural'],
					...
					/**
					 * фильтр для конкретного типа записи
					 */
					register_post_type( $post_type, apply_filters( "wpp_register_{$post_type}_post_types", $args ) );
			...
			if ( ! empty( $taxonomies ) ) :
				...
				foreach ( $taxonomies as $taxonomy => $args ) :
					...
					if ( ! empty( $args['cir'] && 'ru_RU' === get_locale() ) ) {
						$args['labels'] = self::taxonomy_ru_labels( $args );
					} else {
						$args['labels'] = self::taxonomy_labels( $args );
					}
					...
					register_taxonomy( $taxonomy,
						$args['post_types'],
						apply_filters( "wpp_register_{$taxonomy}_taxonomy", $args ) );
		...
		/**
		 * Изменение/добавление сообщений при обновлении/создании записи
		 *
		 * @param $messages
		 *
		 * @return mixed
		...
		public static function unregister_post_types() {
			$default_unregistred = [];
			$unregistred         = apply_filters( 'wpp_unregistred_post_types', $default_unregistred );
		 ...
		 * @return array|mixed
		 */
		public static function images_for_taxonomy( $args ) {
			$taxonomies = self::taxonomies();
			...
			if ( ! empty( $taxonomies ) ) :
				...
				foreach ( $taxonomies as $taxonomy => $args ) :
					...
					if ( ! empty( $args['img'] ) && true === (bool) $args['img'] ) {
						$args[] = $taxonomy;
					}
